...
commands.py: Обработка основных команд (/start, /test, /disable_notifications, /summary, и т.д.).
routers.py: Логика обработки состояний (state machine) для добавления чеков, подтверждения доставки, возвратов и ручного ввода.
notifications.py: Отправка уведомлений о доставке товаров.
sheets.py: Взаимодействие с Google Sheets (проверка пользователей, сохранение чеков, получение баланса).
exceptions.py: Управление списком исключенных товаров.
utils.py: Парсинг QR-кодов и обработка данных чеков.
main.py: Инициализация бота и запуск.
